...
func printRunSummary(label string, result runner.RunResult) {
	...
	if result.TotalInputTokens > 0 || result.TotalOutputTokens > 0 {
		fmt.Fprintf(os.Stderr, "usage:      %s in / %s out\n",
			formatTokensSummary(result.TotalInputTokens),
			formatTokensSummary(result.TotalOutputTokens))
	}
	if result.Error != "" {
...
func formatTokensSummary(n int) string {
	if n >= 1000 {
...
func openRunViewer(runsDir, runID string) error {
	...
	viewNotesPath := filepath.Join(runsDir, runID, "NOTES.md")
	viewProgressPath := filepath.Join(runsDir, runID, "PROGRESS.md")
	model := tui.NewViewerModel(displayEvents, saved.Meta, saved.Prompt, viewNotesPath, viewProgressPath)
	p := newTeaProgram(model, tea.WithAltScreen())
